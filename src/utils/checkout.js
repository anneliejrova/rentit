...
export function sanitizeBookingData() {
  const name = escapeHtml(
    document.querySelector("#checkoutFirstName")?.value.trim(),
  );
  const lastName = escapeHtml(
    document.querySelector("#checkoutLastName")?.value.trim(),
  );
  const email = escapeHtml(
    document.querySelector("#checkoutEmail")?.value.trim(),
  );
  const phone = escapeHtml(
    document.querySelector("#checkoutPhone")?.value.trim(),
  );
  const address = escapeHtml(
    document.querySelector("#checkoutAddress")?.value.trim(),
  );
  const zip = escapeHtml(document.querySelector("#checkoutZip")?.value.trim());
  const city = escapeHtml(
    document.querySelector("#checkoutCity")?.value.trim(),
  );
  ...
  if (!lastName || !nameRegex.test(lastName))
    errors.push("Efternamn (endast bokstäver)");
  if (!email || !emailRegex.test(email))
    errors.push("E-post (ogiltigt format)");
  if (phone && !phoneRegex.test(phone))
    errors.push("Telefonnummer (ogiltigt format)");
